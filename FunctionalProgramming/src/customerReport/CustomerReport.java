...
public class CustomerReport {
	...
	public static List<String> getAllFairfieldCustomers(List<Customer> list) {
		//implement
		return list.stream()
				//as the method returning Customer we have to change it to return string and the map it to return sorted customer names(string)
				.filter(s->s.getCity().equals("Fairfield")).map(cust->cust.getName()).sorted()
